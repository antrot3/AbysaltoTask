...
namespace InfrastructureLayer.Repositories
{
    public class OrderRepository : IOrderRepository
    {
        ...
        {
            var response = await _databaseContext.Orders
        ...
        public async Task<bool> DeleteOrder(int userId)
        {
            var orders = _databaseContext.Orders.Where(c => c.UserId == userId).ToList();
            if(orders.Count<1)
            {
                return false;
            }
